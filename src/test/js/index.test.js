...
describe('JS API', () => {
  test('copy() abs to abs', async () => {
    ...
    assert.equal((await $`cat ${to}/foo.txt`).toString().trim(), 'foo')
  })
  ...
  test('copy() local pattern to local', async () => {
    ...
    assert.equal((await $`cat ${$.cwd}/temp/b/temp/a/foo.txt`).toString().trim(), 'foo')
    try {
  ...
  test('copy() from remote git to local', async () => {
    ...
    assert.equal(JSON.parse((await $`cat ${$.cwd}/temp/package.json`).toString().trim()).name, 'tsc-esm-fix')
    assert.equal(JSON.parse((await $`cat ${$.cwd}/temp/typedoc.json`).toString().trim()).name, 'tsc-esm-fix')
  ...
  test('copy() from git to git', async () => {
    ...
    try {
      ...
      console.log('e', e)
      assert.equal(e.stderr.trim(), `fatal: repository 'https://antongolub.com/fake/repo.git/' not found`)
    }
  })
  ...
  test('copy() from local archive', async () => {
    ...
    assert.equal((await fs.readFile(path.resolve(temp, 'unpacked/foo.js'))).toString('utf8'), 'foo')
  })
  ...
  test('parse()', () => {
    ...
    cases.forEach(([input, expected]) => {
      assert.deepEqual(parse(input, {temp: '<temp>'}), expected)
    })
  })
})
